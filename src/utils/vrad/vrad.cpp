...
bool		g_bGenerateUnitSphereVector = false;
bool		g_bDumpGeneratedUnitSphereVectors = false;
int			g_iUnitSpherePoints = 162;
float		g_fUnitSphereVectorVertexInnerAngle; // Internal parameter, dont touch it!
...
void VRAD_LoadBSP( char const *pFilename )
{
	...
	GenerateUnitSphereLookUpTable(g_bGenerateUnitSphereVector, g_bDumpGeneratedUnitSphereVectors, 
									source, g_iUnitSpherePoints, g_fUnitSphereVectorVertexInnerAngle);
	ParseEntities ();
...
int ParseCommandLine( int argc, char **argv, bool *onlydetail )
{
	*onlydetail = false;
	...
	for( i=1 ; i<argc ; i++ )
	{
		...
		else if (!Q_stricmp(argv[i],"-GenerateUnitSphereVector"))
		{
			if (++i < argc)
			{
				int iTempVal = atoi (argv[i]);
				if (iTempVal < 0)
				{
					Warning("Error: expected non-negative value after '-GenerateUnitSphereVector'\n" );
					return -1;
				}
				iTempVal = (unsigned)iTempVal;
				g_bGenerateUnitSphereVector = true;
				g_iUnitSpherePoints = iTempVal;
			}
			else
			{
				Warning("Error: expected a value after '-GenerateUnitSphereVector'\n" );
				return -1;
			}
		}
		else if (!Q_stricmp(argv[i],"-DumpGeneratedUnitSphereVectors"))
		{
			g_bDumpGeneratedUnitSphereVectors = true;
		}
...
void PrintUsage( int argc, char **argv )
{
	...
	Warning(	
		...
		"  -GenerateUnitSphereVector #: Generates a # number of unit vectors for VRAD to use.\n"
		"								Higher values makes the ambient cubes and detail props  to be lit more acurrate.\n"
		"								(Default: 162. Values above 1024 will make VRAD much slower!)\n"
		"  -DumpGeneratedUnitSphereVectors: Dumps all LUT unit vector to a txt file.\n"
		"\n"
