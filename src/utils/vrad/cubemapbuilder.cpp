...
void BuildCubemaps(const bool bHdrMode)
{
	...
	// GameExecutableName32bits		hl2
	// GameExecutableName64bits		hl2_win64
	const char* pGameExecutableName = pKvGameInfoCubemap->GetString(PLATFORM_64BITS ? CUBEMAPBUILDER_KV_64BITS_EXE : CUBEMAPBUILDER_KV_32BITS_EXE, NULL);

	if (!pGameExecutableName)
	{
		Warning("Warning: Could not locate \'%s\' key %s in %s\n"
				"Warning: Skipping cubemap compile for %s, FAIL!\n",
				PLATFORM_64BITS ? CUBEMAPBUILDER_KV_64BITS_EXE : CUBEMAPBUILDER_KV_32BITS_EXE, 
				pGameExecutableName, 
				szGameInfoPath,
				level_name
		);
