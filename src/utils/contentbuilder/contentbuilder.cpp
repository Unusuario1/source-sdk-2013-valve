...
std::size_t GetSkippedAssets()
{
    ...
    if (!g_buildmaterials)
    {
        i += Shared::CountAssets(g_gamedir, MATERIALSRC_EXTENSION);
    }
    if (!g_buildmodels)
    {
...
void Init_AssetTools()
{
    ...
    {
        Shared::qError("Game dir does not exist, type a valid path in \'-game\'!");
        exit(-1);
    }
    ...
    //scan source assets
    Msg("\tAssetSystem -> Materials: %llu count\n", Shared::CountAssets(g_gamedir, MATERIALSRC_EXTENSION));
