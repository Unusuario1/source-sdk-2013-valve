...
//Builds all the maps given a path.
/*
		MapBuilder
		{
			Vbsp
...
namespace MapBuilder
{
	...
	//-----------------------------------------------------------------------------
	// Purpose: This returns the command line strings of vbsp, vvis & vrad, since maps have 
	// a complex compile we use a custom version of LoadGameInfoKv
	//-----------------------------------------------------------------------------
	void LoadGameInfoKv(char* vbsp_command, char* vvis_command, char* vrad_command, char* vbspinfo_command, std::size_t bufferSize)
	{
		...
		V_snprintf(vbsp_command, bufferSize, " %s %s %s -game \"%s\"  ", "", TOOL_VERBOSE_MODE, vbsp_command, g_gamedir);
		V_snprintf(vvis_command, bufferSize, " %s %s %s -game \"%s\"  ", "", TOOL_VERBOSE_MODE, vvis_command, g_gamedir);
		V_snprintf(vrad_command, bufferSize, " %s %s %s -game \"%s\"  ", "", TOOL_VERBOSE_MODE, vrad_command, g_gamedir);
		V_snprintf(vbspinfo_command, bufferSize, " %s %s %s -game \"%s\"  ", "", TOOL_VERBOSE_MODE, vbspinfo_command, g_gamedir); //Check vbspinfo command line!!
	}
	...
	void CopyBspToGameDir(const char* mapsrc, const char* level_name)
	{
		...
		V_snprintf(mapgamedir, sizeof(mapgamedir), "%s\\%s", g_gamedir, MAPS_DIR);								 // (e.g: "C:\Half Life 2\hl2\maps")
		V_snprintf(mapgamedir_file, sizeof(mapgamedir_file), "%s",
		  /*This causes the a memory leak!*/	Shared::ReplaceSubstring(mapsrc, MATERIALSRC_DIR, MATERIALS_DIR)); // (e.g: "C:\Half Life 2\hl2\maps\test.bsp")
	...
	// What a son of a bitch you are... to many args ._.
	...
	//TODO: Add here exclusion folders!! smth like ExclusionPathVmf { } KV
	//-----------------------------------------------------------------------------
	...
	{
		char vbsp_command[4096] = "", vvis_command[4096] = "", vrad_command[4096] = "", vbspinfo_command[4096] = "", searchPath[MAX_PATH], mapsrcpath[MAX_PATH];
		bool bContinueVmf = true, bContinueVmn = true;

		ColorSpewMessage(SPEW_MESSAGE, &header_color, "\n====== Building %s ======\n", "Maps");

		// We have all the commandline setup of vbsp, vvis, vrad besides the vmf path.
		V_snprintf(searchPath, sizeof(searchPath), "%s\\%s", g_gamedir, MAPSRC_DIR);
		V_snprintf(mapsrcpath, sizeof(mapsrcpath), "%s\\%s", g_gamedir, MAPSRC_DIR); //game/mod/mapsrsc
			
		// TODO: add support for .vmn files!!
		bContinueVmf = Shared::DirectoryAssetTypeExist(searchPath, MAPSRC_EXTENSION, "maps");
		bContinueVmn = Shared::DirectoryAssetTypeExist(searchPath, MAPSRC_EXTENSION2, "maps");
		if (!bContinueVmf && !bContinueVmn)
			return;

		Msg("Asset report:\n");
		Shared::AssetInfoBuild(searchPath, MAPSRC_EXTENSION);
		Shared::AssetInfoBuild(searchPath, MAPSRC_EXTENSION2);
		if (g_infocontent)
			return;
		...
		if (bContinueVmf) 
		{
			MapProcessRec(vbsp_command, vvis_command, vrad_command,
				vbspinfo_command, gamebin, bufferSize,
				searchPath, complete, error, MAPSRC_EXTENSION);
		}		
		if (bContinueVmn)
		{
			MapProcessRec(vbsp_command, vvis_command, vrad_command,
				vbspinfo_command, gamebin, bufferSize,
				searchPath, complete, error, MAPSRC_EXTENSION2);
		}
	}
}
