...
CCoreBuilder::CCoreBuilder(const char* pKeyValue, const char* pFolderNameSrc,
    const char* pFolderNameDst, const bool bPrintAppToTheConsole,
...
// Purpose:   
//-----------------------------------------------------------------------------
CCoreBuilder::CCoreBuilder(const char* pKeyValue, const bool bPrintAppToTheConsole, uint uiThreads, const char* pToolNames)
{
    m_flStartTime           = Plat_FloatTime();
    m_rgpVSrcExtensions     = new FileList();
    m_pVAssetCompileList    = new FileList();
...
void CCoreBuilder::LoadGameInfoKv(char* pCommandLine)
{
    ...
    const char* argv_tool = pKv->GetString(KV_BUILDPARAMS, NULL);
    if (argv_tool)
    {
        V_sprintf_safe(szArgv, " %s ", argv_tool);
    }
    else
    {
        g_pConsoleLogger->Warning("\nAssetSystem%s -> Could not get \'%s\' KeyValues from \"%s\"!\n"
...
void CCoreBuilder::AssetBuilderCompile()
{
    ...
    for (int i = 0; i < len; i++)
    {
        tasks.emplace_back(std::async(std::launch::async,
            ...
            {
                // TODO, change this!! this is so fucking retared!
